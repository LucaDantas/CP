...
struct DSU {
	...
	// isso aq da errado com mod grande, mas pra m <= 10 funciona
	int mx() {
		int ans = 0;
		for(int i = 0; i < maxn; i++)
			ans = max(ans, tam[i]);
...
void join(const vector<piii>& a, const vector<piii>& b) {
	piii bruh = {0, 1, 19};
	bool ok = sz(a) == 3 && a[2] == bruh;
	if(ok) db(a, b);
	static set<int> pos_a, pos_b; pos_a.clear(), pos_b.clear();
	bool ta = 0;
	for(int i = 0, ptr = 0; i < sz(a); i++) {
		while(comp(pos_a, pos_b) == 1 && ptr < sz(b))
			add(pos_b, b[ptr].sz), ++ptr, ta = 1;
		add(pos_a, a[i].sz);
		if(!a[i].cor) continue;
		if(ok) db(pos_a, pos_b, ptr, ta, sz(b));
		while(ptr-ta < sz(b) && comp(pos_a, pos_b) != 0) {
			if(ok) db("OPA");
			if(b[ptr-ta].cor)
				dsu.join(a[i].t, b[ptr-ta].t);
			if(!ta) add(pos_b, b[ptr].sz);
			if(ta) ta = 0;
			else ++ptr;
		}
	}
