// Optimized with two pointers instead of doing a ternary search for every query
#include<bits/stdc++.h>
using namespace std;
...
int main() {
	...
	for(int k = 0; k < m; k++) {
		...
		for(int i = 0; i < n; i++) {
			...
			// I don't want collinear points because I don't want to check for repeated points
			while(hull[k].size() >= 2 && !cw(hull[k][(int)hull[k].size()-2], hull[k].back(), pts[i]))
				hull[k].pop_back();
			hull[k].push_back(pts[i]); mx = max(mx, pts[i].y);
		}
	...
	for(int i = 0, A, B; i < Q; i++)
		scanf("%d %d", &A, &B), queries[i] = Pt(A, B, i);
	sort(queries, queries+Q, [](Pt a, Pt b) {
		return 1ll * a.x * b.y < 1ll * b.x * a.y;
	});
	// auto search = [&](Pt p, int k) {
	...
	// };
	for(int k = 0; k < m; k++) {
		if(!hull[k].size()) break;
		int ptr = 0;
		for(int i = 0; i < ((int)hull[k].size())-1; i++)
			for(; ptr < Q && queries[ptr] * hull[k][i] > queries[ptr] * hull[k][i+1]; ptr++)
				bom[k][queries[ptr].id] = i;
		for(; ptr < Q; ptr++)
			bom[k][queries[ptr].id] = ((int)hull[k].size())-1;
	}
