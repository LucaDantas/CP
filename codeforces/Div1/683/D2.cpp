...
int main() {
	...
	for(int i = 0; i < n; i++) {
		if(a[i] == top) {
			...
			if(last[a[i]] != -1) {
				k = dps[last[a[i]]];
				if(k != -1)
					for(int rep = 0; k < sz(ultimos) && rep <= cnt[a[i]]; k++, rep++)
						seq[a[i]].pb({-1, ultimos[k]});
				else k = 0;
			}

			int j = 1;
			vector<pii> add;
			for(; j <= cnt[a[i]]+1 && sz(ultimos)-j >= k && ultimos[sz(ultimos)-j] > last[a[i]]; j++)
				add.pb({-1, ultimos[sz(ultimos)-j]});
			reverse(all(add));
			for(pii x : add)
				seq[a[i]].pb(x);
	...
	for(int i = 1; i <= n; i++) {
		if(last[i] != -1) {
			int k = dps[last[i]];
			if(k != -1)
				for(int rep = 0; k < sz(ultimos) && rep <= cnt[i]; k++, rep++)
					seq[i].pb({-1, ultimos[k]});
		}
	}
	int ans = 0;
	for(int i = 1; i <= n; i++) {
