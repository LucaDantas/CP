...
int main() {
	...
	while(t--) {
		int n, k; rd(n, k);
		vi a(n); rep(i,0,n) rd(a[i]);
		vi w(k); rep(i,0,k) rd(w[i]);
		sort(all(a));
		ll ans = 0;
		int cnt = k, ones = 0;
		rep(i,0,k) if(w[i] == 1) ones++;
		while(cnt--) {
			ans += a.back();
			if(ones) {
				ones--; ans += a.back(); 
			}
			a.pop_back(); // the greatest values are in the end
			w[cnt]--; // we reduce one from the intergers that guy want
		}
		sort(all(w), greater<int>()); // we'll give greedely the smallest values to the one
		// who wants more values
		int pos = 0;
		rep(i,0,k) {
			if(w[i] == 0) continue;
			ans += a[pos];
