...
// first type selects the one that has more cookies, if they're equal choose a
// second type selects the one that has fewer cookies, if they're equal choose b

int main() {
	int t; rd(t);
	while(t--) {
		ll a, b, n, m; rd(a, b, n, m);
		if(m <= min(a,b) && n <= a+b-m) wt("Yes");
		else wt("No");
	}
}
