...
int main() {
	...
	while(t--) {
		...
		// we build it cyclically, since n² is small we can go on building
		// building in every diagonal
		int pos = 0, cnt = 0;
		while(k--) {
			grid[pos][(pos+cnt)%n] = 1;
			pos++;
			if(pos >= n) {
				pos = 0;
				cnt++;
		...
		int mx_row = 0, mn_row = inf, mx_col = 0, mn_col = inf, r_now = 0, c_now = 0;
		rep(i,0,n) {
			rep(j,0,n) {
				r_now += grid[i][j];
				c_now += grid[j][i];
			}
			mx_row = max(mx_row, r_now);
			mn_row = min(mn_row, r_now);
			mx_col = max(mx_col, c_now);
			mn_col = min(mn_col, c_now);
			r_now = 0;
			c_now = 0;
		}
		printf("%lld\n", 1ll*(mx_row - mn_row)*(mx_row - mn_row) + 1ll*(mx_col - mn_col)*(mx_col - mn_col));
		rep(i,0,n) {rep(j,0,n) printf("%d", grid[i][j]); printf("\n");}
	}
}
