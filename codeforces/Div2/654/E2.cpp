...
int main() {
	...
	int l = 0;
	rep(i,0,n) l = max(l, a[i] - i);

	int mx = 0;
	rep(i,0,n) {
		int cnt = (int)(upper_bound(all(a), l+i) - a.begin());
		if(i >= p - (cnt-i)%p) mx = max(mx, (cnt-i)%p);
		if((cnt-i)%p == 0) {puts("0"); return 0;}
	}
	mx = max(p-mx, 0); // what's missing to get to the prime
