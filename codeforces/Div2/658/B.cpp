...
bool solve(int pos, int mv, vector<vi>& dp, const vi& a) {
	if(pos == (sz(a)-1)) return true;
	if(dp[pos][mv] != -1) return dp[pos][mv];
	// db(pos, mv);
	if(mv) {
		bool l = solve(pos+1, a[pos+1]>1, dp, a);
		bool r = solve(pos, 0, dp, a);
		bool ans = !(l && r);
		// db(pos, ans);
		return (dp[pos][mv] = ans);
	}
	else return (dp[pos][mv] = (!solve(pos+1, a[pos+1]>1, dp, a)));
}
...
int main() {
	...
	while(t--) {
		...
		wt(solve(0, a[0] > 1, dp, a)?"First":"Second"); //cerr << endl;
		// db(dp); cerr << endl;
	}
}
