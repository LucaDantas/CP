...
int main() {
	int n, m; rd(n, m);
	int x, range, y; rd(x, range, y);
	vi a(n), b(m); rd(a, b);

	int pos = 0;
	vector<pair<vi, int>> itv;
	vi now;
	...
	rep(i,0,sz(itv)) {
		...
		int edge = itv[i].ss;
		int tam = sz(v);
		int maior = *max_element(all(v));

		if(tam < range && maior > edge) {wt("-1"); return 0;}

		ll use = inf;

		rep(q,0,tam / range + 1) {
			if(q == 0 && maior > edge) continue;
			use = min(use, 1ll*q*x + 1ll*(tam - q*range)*y);
		}
