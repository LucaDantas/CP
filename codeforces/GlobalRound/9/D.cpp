...
int find_element(int v, const vi& a) {
	int n = sz(a);
	rep(i,0,n) if(a[i] == v) return i;
	// db(v);
	assert(0);
	return 0;
}

int first(const vi& a) {
	int n = sz(a);
	rep(i,0,n) if(a[i] == 0) return i;
	assert(0);
	return 0;
}
