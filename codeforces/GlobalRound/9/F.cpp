...
// What we do here is follow a simple formula that breaks the opponent
// a, b, c -> add a+b+c -> 2a+b+c, b, c (simetric, so if not chosen the first swap)
// 2a+b+c, b, c -> add 4a+b+c -> 2a+b+c, 4a+2b+c, c (simetric... same as above)
// 2a+b+c, 4a+2b+c, c -> add 2a+b, it breaks whatever the opponent chooses

int main() {
	vi v(3), ini;
	rd(v); ini = v;
	ll sum = 0; rep(i,0,3) sum += v[i];

	wt("First"); flush();
	wt(sum); flush();
	int nxt; rd(nxt);
	if(nxt != 1)
		swap(ini[0], ini[nxt-1]);
	wt(4ll*ini[0] + ini[1] + ini[2]); flush();
	rd(nxt);
	if(nxt == 3) 
		swap(ini[1], ini[2]);
	wt(2ll*ini[0] + ini[1]); flush();
	rd(nxt);
	assert(nxt == 0);
}
